...
import { NotificationService, ArtifactNotificationData } from '../services/notification-service';
    ...
    lastMessage.content.forEach(fragment => {
      ...
      if (ARTIFACT_TYPES.includes(fragment.type)) {
        ...
        if ('audioId' in fragment) {
          artifactId = fragment.audioId as string;
        } else if ('butcherId' in fragment) {
          artifactId = fragment.butcherId as string;
        }
